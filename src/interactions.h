...
__device__ glm::vec3 bxdf_frensel_specular_sample_f(const glm::vec3& wo, glm::vec3* wi, const glm::vec2& random, float* pdf, glm::vec3 reflectionAlbedo, glm::vec3 refractionAlbedo, glm::vec2 refIdx)
{
	...
	if (random.x < frensel)
	{
		//printf("always in frensel random < ");
...
__device__ float lights_sample_pdf(const SceneInfoDevice& sceneInfo, int lightPrimID)
{
...
__device__ void lights_sample(const SceneInfoDevice sceneInfo, const glm::vec3& random, const glm::vec3& position, const glm::vec3& normal, glm::vec3* lightPos, glm::vec3* lightNormal, glm::vec3* emissive, float* pdf)
{
	//printf("sceneInfo\n");
	if (sceneInfo.lightsSize == 0) return;
	//printf("sceneInfo lightsize\n");
